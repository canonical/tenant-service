...
- Remove the local OpenAPI definition files (`openapi/openapi.swagger.json`, `openapi/openapi.yaml`).
- Remove `buf.gen.yaml` and `buf.yaml` (no longer needed for local code generation).
- Update `go.mod` to depend on `github.com/canonical/identity-platform-api` at the `IAM-1998` branch commit (to be switched to `main` once merged).
...
- **Files removed**: `v0/`, `api/`, `openapi/`, `buf.gen.yaml`, `buf.yaml`
