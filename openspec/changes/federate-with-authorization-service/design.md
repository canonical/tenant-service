...
- **Rationale**: The endpoint `GET /api/v0/me/tenants` returns the calling user's tenant memberships and only inspects the caller's own memberships (`authentication.GetUserID(ctx)`). Rather than maintaining an artificial `account:me` OpenFGA resource and publishing static wildcard tuples (`user:* -> can_view -> account:me`) on startup, the Istio Gateway bypasses external authorization for this path. The `HTTPRoute` routes `/api/v0/me/tenants` to `tenant-service`, but the `ext_authz` `AuthorizationPolicy` lists only the protected tenant-service paths (`/api/v0/tenants`, `/api/v0/tenants/*`, `/api/v0/users`, `/api/v0/users/*`) in `paths`, so `/api/v0/me/tenants` is never sent to `authorization-service`. The built-in JWT authentication middleware in `tenant-service` validates the token and extracts the user identity. Scoping the policy to explicit tenant-service `paths` also avoids matching other services' routes on the shared gateway.
